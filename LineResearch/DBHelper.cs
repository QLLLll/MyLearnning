...
namespace LineResearch
{
    ...
    public static class DBHelper
    {
        ...
        /// <param name="key"></param>
        /// <param name="db"></param>
        public static Xrecord GetNODData(string name, string key, Database db = null)
        {
            db = db ?? Application.DocumentManager.MdiActiveDocument.Database;
            ...
            using (var trans = db.TransactionManager.StartOpenCloseTransaction())
            {
                ...
                var dict = trans.GetObject(nod.GetAt(name), OpenMode.ForRead) as DBDictionary;
                if (dict.Contains(key))
                {
                    return trans.GetObject(dict.GetAt(key), OpenMode.ForRead) as Xrecord;
                }
