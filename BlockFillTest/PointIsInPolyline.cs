...
namespace BlockFillTest
{
    public class PointIsInPolyline
    {
        ...
        /// </summary>
        public int PtRelationToPoly(Polyline pPoly, Point2d pt, double tol)
        {
        ...
        /// 返回值：-1表示在多段线外部，0表示在多段线上，1表示在多段线内部
        /// </summary>
        public int PtRelationToPoly(Polyline pPoly, Point3d pt, double tol)
        {
            Debug.Assert(pPoly != null);

            // 1.如果点到多段线的最近点和给定的点重合，表示点在多段线上
            Point3d closestPoint = pPoly.GetClosestPointTo(pt, false);		// 多段线上与给定点距离最近的点	
            if (Math.Abs(closestPoint.X - pt.X) < tol && Math.Abs(closestPoint.Y - pt.Y) < tol)			// 点在多段线上
            {
                return 0;
            }
